...
FastAPI 주소가 바뀌면 `API_BASE_URL` 값을 수정하세요. 데이터는 페이지 최초 접속, 저장·삭제 완료, 사용자가 새로고침 버튼을 눌렀을 때 갱신됩니다.
...
- 물 섭취량과 소변량을 서로 독립적으로 추가, 수정, 삭제
- 물 섭취량과 소변량의 최근 값, 오늘 누계, 개별 그래프
- 이벤트 기록 추가, 수정, 삭제
- 치료 및 할 일 추가, 수정, 삭제, 완료, 취소
- 교대 인계 메모 저장 및 인계 내용 복사
- 전체 JSON 및 SQLite 백업 내려받기
- 수동 새로고침 및 저장·삭제 후 갱신
